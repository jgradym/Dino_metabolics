...
theme_jg <- theme(panel.grid = element_blank(),
                    aspect.ratio = .75,
                    axis.text = element_text(size = 18, color = "black"), #12
                    axis.ticks.length = unit(0.2, "cm"),
                    axis.title = element_text(size = 18), #12
...
```{r, echo=F, warning=F, message=F}
dat.lm <- read.csv(file=paste(getwd(), "/data/dino_MR_lm_jg.csv", sep=""), header=T, stringsAsFactors = T)

dat.calib <- read.csv(file=paste(getwd(), "/data/calib.csv", sep=""), header=T, stringsAsFactors = T)

dat.calib.extant <- read.csv(file=paste(getwd(), "/data/calib_25.csv", sep= "" ), header=T, stringsAsFactors = T)

dat.calib.fossil <- read.csv(file=paste(getwd(), "/data/calib_fossil.csv", sep=""), header=T, stringsAsFactors = T)
...
tree <- read.nexus(file=paste(getwd(), "/data/consensus_Paleotree_final.tre", sep=""))

tree.tt <- ape::read.tree(file=paste(getwd(), "/data/extant.nwk", sep=""))

# disc.df <-read.csv(file=paste(getwd(), "data/disc_fossil.csv", sep=""), header=T, stringsAsFactors = T)
...
```
## Plot Scaling of Ratios
```{r, fig.dim=c(8,6), echo=F, warning=F, message=F}
library(readr)
ratios <- read_csv(file = paste(getwd(), "/data/combo_data.csv", sep = "")) 

#no Ratio log transformed
ggplot(ratios, aes(x = Mass_kg, y = Amide_ratio)) +
  geom_point(shape = 21, aes(fill = Taxa3), size = 3, color = "black") +
 scale_x_log10(name = "Body Mass (kg)", breaks = c(1, 10, 100, 1000, 10000), 
                labels = c("1", "10", "100", "1000", "10,000")) + 
  scale_y_continuous("Amide Ratio") +
  scale_fill_manual(values = c("Endotherm" = "brown3", "Ectotherm" = "dodgerblue", "Dinosaur" = "gray", Pterosaur = "black", "Plesiosaur" = "white", "Synapsid" = "gray30")) +
   scale_color_manual(values = c("Endotherm" = "brown3", "Ectotherm" = "dodgerblue", "Unknown" = "gray")) +
  theme_jg +
   stat_smooth(data = . %>% filter(Taxa3 == "Endotherm" | Taxa3 == "Ectotherm" | Taxa3 == "Dinosaur"), method = "lm", aes(color = Taxa3, fill = Taxa3), alpha = 0.1) + 
...
ggplot(ratios, aes(x = Mass_kg, y = Amide_ratio)) +
  scale_x_log10(name = "Body Mass (kg)", breaks = c(1, 10, 100, 1000, 10000), 
                labels = c("1", "10", "100", "1000", "10,000")) + 
  scale_y_log10("Amide Ratio", breaks = c(2, 3, 5, 10)) +
   scale_fill_manual(values = c("Endotherm" = "brown3", "Ectotherm" = "dodgerblue", "Dinosaur" = "gray", Pterosaur = "black", "Plesiosaur" = "white", "Synapsid" = "gray40")) +
   scale_color_manual(values = c("Endotherm" = "brown3", "Ectotherm" = "dodgerblue", "Dinosaur" = "gray", Pterosaur = "black", "Plesiosaur" = "white", "Synapsid" = "gray40")) +
  stat_smooth(data = . %>% filter(Taxa3 == "Endotherm" | Taxa3 == "Ectotherm" | Taxa3 == "Dinosaur"), method = "lm", aes(color = Taxa3, fill = Taxa3), alpha = 0.1) + 
  geom_point(shape = 21, aes(fill = Taxa3), size = 4, color = "black") +
  theme(legend.key = element_rect(fill = "transparent")) + # doesn't work
  theme_jg + theme(legend.key=element_blank()) +  
  guides(fill = guide_legend(override.aes = list(linetype = 0)),
         color = guide_legend(override.aes = list(linetype = 0)))
#would like to remove second Taxa3 legend, remove legend header ("Taxa3"), and box around points, option to add prediction intervals
```
...
ggplot(orig_df_full, aes(Log.10..Body.Mass..g., Log.10..Upper.MR))+
 ...
 # geom_smooth(data=orig_df_endo, method="lm", fill="goldenrod1", linetype=0)+
     geom_ribbon(data=orig_df_endo,aes(ymin=lwr, ymax = upr), alpha = 0.3, fill = "brown3")+
    #geom_ribbon(data=orig_df_endo, aes(ymax=upr), color = "brown3", linetype = "dashed")+
 # geom_smooth(data=orig_df_oth, method="lm", fill="dodgerblue2", linetype=0)+
    ...
    #geom_line(data=orig_df_oth, aes(y=upr), color = "dodgerblue2", linetype = "dashed")+
      geom_ribbon(data=orig_df_oth, aes(ymax = upr, ymin = lwr), alpha = 0.3, fill = "dodgerblue2")+

  geom_point(data= dat.lm, mapping= aes(Log.10..Body.Mass..g., Log.10..Upper.MR, fill=thermy), shape=21, size = 4)+
...
ggplot(data=dat.calib.extant%>%filter(classification %in% c("endo","ecto")), aes(Log.10..Body.Mass..g., log10(Literature.MR..mL.O2...1.h...1.g.), fill =classification, color = classification))+
 ...
 # theme_classic()+
  ...
  labs(x=expression(paste(Log["10"]~Body~Mass~(g))), 
       y= expression(paste(Log["10"]~"Literature Mass-Specific MR (ml", O[2]*h^-1*g^-1, ")")))
  

```

We see that known endotherms in red show a slightly positive slope barely greater than zero—running counter to the expected -1/4 slope derived from previous work. Ectotherms actually do show a nearly -1/4 slope but the association is clearly driven by a single point, making inference dubious.
...
ggplot(data=dat.calib%>%filter(!row_number() %in% c(9)), aes(X.S.N..Amide...MC.in.vivo, Literature.MR..mL.O2...1.h...1.g.))+
  geom_point(size = 4)+
  geom_smooth(method="lm", shape = 21, color="black", fill= "grey88", alpha=0.4, se=F)+
  geom_point(size = 4, shape = 21, color = "black", data=dat.calib%>%filter(!row_number() %in% c(8)), 
             aes(X.S.N..Amide...MC.in.vivo, Literature.MR..mL.O2...1.h...1.g.), fill ="blue")+
  geom_smooth(data=dat.calib%>%filter(!row_number() %in% c(8)), aes(X.S.N..Amide...MC.in.vivo, Literature.MR..mL.O2...1.h...1.g.),
              color="blue", method="lm", se=F)+
  theme_jg +
#theme_classic()+
  labs(x=expression(paste("Relative Intensity ([S-hcs] + [N-hcs]) x ", Amide^{-1})), 
       y= expression(paste("Mass-Specific MR (",O[2]*h^-1*g^-1, ")")))+
  geom_label(
    label="r2 = 0.04 \n corrected", 
    x=10,
    y=1.5,
...
```
Hard to read individual labels but see figure 1 in reply for neater representation. It is clear from this plot that most data in the middle of the MC vivo range (steep slope) has very low prediction power.

### Check predictive power of model using the fossil taxa of other thermy
```{r, warning=F, message=F}
pred.foss.oth <- predictions(MR.only,
                             newdata = datagrid(
                               log10_adj_corr_MR.z = dat.brms %>% filter(Age == "Fossil", classification ==
                                                                           "oth") %>% pull(log10_adj_corr_MR.z)
                             ))

pred.foss.oth <- posteriordraws(pred.foss.oth)

oth.tax <-
  dat.brms %>% filter(Age == "Fossil", classification == "oth") %>% dplyr::select(Taxon, log10_adj_corr_MR.z) %>%
  pull(Taxon)
...
pred.foss.oth <-
  left_join(
    pred.foss.oth,
    full.analyses.df %>% filter(Age == "Fossil", classification == "oth") %>%
      dplyr::select(Taxon, termy_3_cat) %>% dplyr::rename(taxon = Taxon)
  )

oth.pr <- pred.foss.oth %>%
  group_by(taxon) %>%
  summarize(
    log10_adj_corr_MR.z = median(log10_adj_corr_MR.z),
    draw = median(draw),
    median = median(draw),
    uncertainty = conf.high - conf.low
  )
...
pred.foss.endo <- predictions(MR.only,
                              newdata = datagrid(
                                log10_adj_corr_MR.z = dat.brms %>% filter(Age == "Fossil", classification ==
                                                                            "endo") %>% pull(log10_adj_corr_MR.z)
                              ))

pred.foss.endo <- posteriordraws(pred.foss.endo)

endo.tax <-
  dat.brms %>% filter(Age == "Fossil", classification == "endo") %>% dplyr::select(Taxon, log10_adj_corr_MR.z) %>%
  pull(Taxon)
...
anc.med<-as.matrix(anc.df)[,1] # ASR with median model estimates
anc.med

n_cols <- n_distinct(anc.med)

obj <- contMap(tree.anc, anc.med, res=500, lwd=1)

# Change the color palette
contmap_obj_viridis <- setMap(obj, rocket(n_cols))
...
anc.var<-as.matrix(anc.df)[,2] # ASR with credible interval span
anc.var

n_cols <- n_distinct(anc.var)

obj2 <- contMap(tree.anc, anc.var, res=500, lwd=1)

# Change the color palette
contmap_obj2_viridis <- setMap(obj2, rocket(n_cols))

# Plot the mapped characters with the new colors
plot(
  contmap_obj2_viridis,
  lwd=2,
  legend = 0.7*max(nodeHeights(tree.anc)),
  fsize = c(0.5, 0.7))
...
plot(
  ...
  lwd=2,
  legend = 0.7*max(nodeHeights(tree.anc)),
  fsize = c(0.5, 0.7), direction="leftwards")
