#Analyses Metabolic rates #NOTE THIS IS DOWNLOADED FROM IRIS MENDEZ' GitHub Page on 8/13/2022
#R version 3.5.1
...
#Read the topology
phy <- read.nexus("~/Dropbox/Research/Dino_metabolics_rebuttal/data/Final_Tree_Iris_Mendez.nex")


#Read the stratigraphic ranges of species (FAD and LAD)
taxonTimes <- read.table("~/Dropbox/Research/Dino_metabolics_rebuttal/data/taxon_times.txt",header=TRUE,row.names=1)
#Read the minimum and maximum ages of each time interval
intervalTimes <- read.table("~/Dropbox/Research/Dino_metabolics_rebuttal/data/IntervalTimes.txt",header=TRUE,row.names=1)
...
pdf("~/Dropbox/Research/Dino_metabolics_rebuttal/figures/nodelabels.pdf")
plot(phy, cex=0.5)
...
pdf("~/Dropbox/Research/Dino_metabolics_rebuttal/figures/scaled_phylo_final.pdf")
geoscalePhylo(Consensus_phy_lad,units=c("Period", "Epoch", "Age"), cex.tip=0.2, width= 1, show.tip.label=T)
dev.off()


writeNexus(Consensus_phy_lad, "~/Dropbox/Research/Dino_metabolics_rebuttal/data/consensus_Paleotree_final.tre")
